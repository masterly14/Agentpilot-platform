...
import { transitionPipeline } from "@/lib/pipeline/engine"
...
export async function POST(request: Request) {
  ...
  if (contactId) {
    try {
      await transitionPipeline({ contactId, toState: "NO_SHOW" })
    } catch (error) {
      console.warn("[pipeline] no-show sin pipeline", contactId, error)
    }
  }
