...
import { FEATURED_TESTIMONIAL } from "@/components/social-proof/content"
import { AvatarSlot, MediaSlot, Reveal, SectionHeading } from "@/components/social-proof/primitives"
import { DIAGNOSIS_PROOF } from "./content"

export function DiagnosisTestimonial() {
  const testimonial = FEATURED_TESTIMONIAL

  return (
    <section id="prueba-real" className="relative scroll-mt-8 overflow-hidden">
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_50%_50%,rgba(6,182,212,0.10),transparent_65%)]"
      />

      <DashedGrid gridId="diag-proof" maxWidth="6xl" padding="px-4 py-16 md:px-6 md:py-24">
        <SectionHeading
          eyebrow={DIAGNOSIS_PROOF.badge}
          titleLead={DIAGNOSIS_PROOF.titleLead}
          titleAccent={DIAGNOSIS_PROOF.titleAccent}
          description={DIAGNOSIS_PROOF.description}
...
function VideoSlot({ src, company }: { src?: string; company: string }) {
  if (src) {
    return (
