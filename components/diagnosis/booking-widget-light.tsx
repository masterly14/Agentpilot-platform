  ...
  if (isUnavailable || isDisabled) {
    return (
      <div className="flex aspect-square w-full items-center justify-center">
        <span className={cn("text-sm", cell.isOutside ? "text-zinc-300" : "text-zinc-400")}>
          {cell.day}
        </span>
      </div>
    )
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between">
        <span className="text-sm font-medium text-zinc-900">{formatSelectedDate(viewYear, viewMonth, selectedDay)}</span>
      ...
      >
        ...
        >
          <div className="mb-4 flex items-center justify-between md:mb-5">
            <span className="text-sm font-medium text-zinc-900">{bookingMonthLabel(viewYear, viewMonth)}</span>
            <div className="flex items-center gap-1">
              <button
                type="button"
                onClick={() => handleShiftMonth(-1)}
                disabled={!canGoPrev || isLoadingCalendar}
                className="flex h-7 w-7 items-center justify-center text-zinc-500 hover:text-zinc-900 disabled:cursor-not-allowed disabled:text-zinc-300 disabled:hover:text-zinc-300"
